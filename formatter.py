...
def unpack(bin_file, data_type, length_arg=0):
    ...
    elif data_type == "l" or data_type == "L":
        return int(struct.unpack(data_type, bin_file.read(4))[0])
    elif data_type == "q" or data_type == "Q":
        return int(struct.unpack(data_type, bin_file.read(8))[0])
    #float
    elif data_type == "f":
...
#get a raw value from bin_list, which contains an offset and a form for how to interpret data in the file at that offset
